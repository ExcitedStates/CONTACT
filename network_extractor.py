...
colors = ['firebrick','steelblue','seagreen','darkorange','yellow','orange','purple','grey','blue']
i = 0
...
for g in C:
    ...
    if len(resis) > 2:
        ...
        #print max_value
        size = [ 250*(degree[n]+1.0) for n in nodes]
        print size
        #max_index = degree.index(max_value)
        #pos=nx.spring_layout(g,weight=0.1)
        if len(resis) > 20:
           pos=layout(g,prog='twopi', root='153')
        else:
           pos=layout(g,prog='twopi')
        nx.draw_networkx_nodes(g,pos,node_size=size,node_color=colors[i])
        nx.draw_networkx_labels(g,pos,font_size=11,font_color='white', font_family='sans-serif')
        for e in g.edges():
#             print e, G.get_edge_data(e[0],e[1])['weight']
             nx.draw_networkx_edges(G,pos,edgelist=[e],width=(G.get_edge_data(e[0],e[1])['weight'])/5000+1,color=colors[i] )#for cypa, remove **0.5, for dhfr include
        print "color %s, resi %s" %(colors[i],"+".join(resis))
        print "create %s_sec, resi %s" %(colors[i],"+".join(resis))
        print "show surface, %s_sec" %(colors[i])
        plt.savefig("%s.svg" %colors[i], format="svg")
        plt.clf()    
        i+=1
        if i > 8:
            i = 0
